...
function dualListBox() {
    ...
    this.configureBoxes = function(options) {
        ...
        jQuery.extend(settings[index], options);
        ...
        if (settings[index].useFilters) {
            jQuery('#' + group1[index].filter).keyup(function() {
                Filter(group1[index]);
            });
            jQuery('#' + group2[index].filter).keyup(function() {
                Filter(group2[index]);
            });
            jQuery('#' + group1[index].clear).click(function() {
                ClearFilter(group1[index]);
            });
            jQuery('#' + group2[index].clear).click(function() {
                ClearFilter(group2[index]);
            });
        }
        if (IsMoveMode(settings[index])) {
        	jQuery('#' + group2[index].view).dblclick(function() {
                MoveSelected(group2[index], group1[index]);
            });
        	jQuery('#' + settings[index].to1).click(function() {
                MoveSelected(group2[index], group1[index]);
            });
        	jQuery('#' + settings[index].allTo1).click(function() {
        ...
        jQuery('#' + group1[index].view).dblclick(function() {
            MoveSelected(group1[index], group2[index]);
        });
        jQuery('#' + settings[index].to2).click(function() {
            MoveSelected(group1[index], group2[index]);
        });
        jQuery('#' + settings[index].allTo2).click(function() {
        ...
        jQuery('#' + group1[index].storage + ',#' + group2[index].storage).css('display', 'none');

        //attach onSubmit functionality if desired
        if (settings[index].selectOnSubmit) {
        	jQuery('#' + settings[index].box2View).closest('form').submit(function() {
        		jQuery('#' + settings[index].box2View).children('option').attr('selected', 'selected');
            });
        	jQuery('#' + settings[index].box1View).closest('form').submit(function() {
        		jQuery('#' + settings[index].box1View).children('option').attr('selected', 'selected');
            });
        }
    };

    function UpdateLabel(group) {
        var showingCount = jQuery("#" + group.view + " option").size();
        var hiddenCount = jQuery("#" + group.storage + " option").size();
        jQuery("#" + group.counter).text('Showing ' + showingCount + ' of ' + (showingCount + hiddenCount));
    }
    ...
    function Filter(group) {
        ...
        var filterLower;
        if (settings[index].useFilters) {
            filterLower = jQuery('#' + group.filter).val().toString().toLowerCase();
        } else {
            filterLower = '';
        }
        jQuery('#' + group.view + ' option').filter(function(i) {
            var toMatch = jQuery(this).text().toString().toLowerCase();
            return toMatch.indexOf(filterLower) == -1;
        }).appendTo('#' + group.storage);
        jQuery('#' + group.storage + ' option').filter(function(i) {
            var toMatch = jQuery(this).text().toString().toLowerCase();
            return toMatch.indexOf(filterLower) != -1;
        }).appendTo('#' + group.view);
        try {
        	jQuery('#' + group.view + ' option').removeAttr('selected');
        }
        catch (ex) {
    ...
    function SortOptions(group) {
        var $toSortOptions = jQuery('#' + group.view + ' option');
        $toSortOptions.sort(onSort[group.index]);
        jQuery('#' + group.view).empty().append($toSortOptions);
    }

    function MoveSelected(fromGroup, toGroup) {
        if (IsMoveMode(settings[fromGroup.index])) {
        	jQuery('#' + fromGroup.view + ' option:selected').appendTo('#' + toGroup.view);
        } else {
        	jQuery('#' + fromGroup.view + ' option:selected:not([class*=copiedOption])').clone().appendTo('#' + toGroup.view).end().end().addClass('copiedOption');
        }
        try {
        	jQuery('#' + fromGroup.view + ' option,#' + toGroup.view + ' option').removeAttr('selected');
        }
        catch (ex) {
            //swallow the error for IE6
        }
    ...
    function MoveAll(fromGroup, toGroup) {
        if (IsMoveMode(settings[fromGroup.index])) {
        	jQuery('#' + fromGroup.view + ' option').appendTo('#' + toGroup.view);
        } else {
        	jQuery('#' + fromGroup.view + ' option:not([class*=copiedOption])').clone().appendTo('#' + toGroup.view).end().end().addClass('copiedOption');
        }
        try {
        	jQuery('#' + fromGroup.view + ' option,#' + toGroup.view + ' option').removeAttr('selected');
        }
    ...
    function RemoveSelected(removeGroup, otherGroup) {
    	jQuery('#' + otherGroup.view + ' option.copiedOption').add('#' + otherGroup.storage + ' option.copiedOption').remove();
        try {
        	jQuery('#' + removeGroup.view + ' option:selected').appendTo('#' + otherGroup.view).removeAttr('selected');
        }
        catch (ex) {
            //swallow the error for IE6
        }
        jQuery('#' + removeGroup.view + ' option').add('#' + removeGroup.storage + ' option').clone().addClass('copiedOption').appendTo('#' + otherGroup.view);
        Filter(otherGroup);
    ...
    function RemoveAll(removeGroup, otherGroup) {
    	jQuery('#' + otherGroup.view + ' option.copiedOption').add('#' + otherGroup.storage + ' option.copiedOption').remove();
        try {
        	jQuery('#' + removeGroup.storage + ' option').clone().addClass('copiedOption').add('#' + removeGroup.view + ' option').appendTo('#' + otherGroup.view).removeAttr('selected');
        }
        catch (ex) {
            //swallow the error for IE6
        }
    ...
    function ClearFilter(group) {
    	jQuery('#' + group.filter).val('');
    	jQuery('#' + group.storage + ' option').appendTo('#' + group.view);
        try {
        	jQuery('#' + group.view + ' option').removeAttr('selected');
        }
        catch (ex) {
